 ...
 */
	...
	foreach ($assistedcommands_categories as $key => $category) {		
		...
		try {
			...
			notify($vce->getMessage());
		}
	}
	
	// Display link to the advanced mode
	echo '<br/><center>';
	print_single_button('view.php', array('view' => 'sadmin', 'what' => 'switchtoadvancedcommand'), get_string('advancedmode', 'block_vmoodle'));
	echo '<br/>';
	print_single_button('admin.php', array(), get_string('administration', 'block_vmoodle'));
	echo '</center>';