...
function vmoodle_config_get_plugins_params() {
	...
	$paramslist = $DB->get_records_sql_menu("SELECT DISTINCT id,CONCAT(plugin,'/',name) FROM {$CFG->prefix}config_plugins");
...
function vmoodle_config_get_params() {
	...
	$paramslist = $DB->get_records_sql_menu("SELECT DISTINCT id,name FROM {$CFG->prefix}config");
	$paramlist = array_combine(array_values($paramslist), array_values($paramslist));
	return $paramlist;
}
