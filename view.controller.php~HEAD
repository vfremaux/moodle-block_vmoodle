...
if ($action == 'doaddupdate'){
    ...
    $form->vdbprefix = required_param('vdbprefix', PARAM_CLEAN);
    ...
    $vmoodle->description = addslashes($form->description);
    ...
    if(empty($form->shortname) || empty($form->vhostname)){
        ...
        $errors[] = $erroritem;
    } elseif (get_record('block_vmoodle', 'name', $form->name)) {
        $erroritem->message = get_string('hostexists', 'block_vmoodle');
        $erroritem->on = 'name';
        $errors[] = $erroritem;
    } elseif (get_record('block_vmoodle', 'shortname', $form->shortname)) {
        $erroritem->message = get_string('shortnameexists', 'block_vmoodle');
        $erroritem->on = 'shortname';
        $errors[] = $erroritem;
    } elseif (get_record('block_vmoodle', 'vhostname', $form->vhostname)) {
            ...
            if (!$side_cnx){
                ...
                if ($form->mnet){
                    ...
                    if ($CFG->mnet_dispatcher_mode == 'strict'){
                        ...
                        $mnet_moodles = get_records('block_vmoodle', 'mnet', 1);
    ...
    if (empty($errors)){
        ...
        if ($form->id){
            ...
            insert_record('block_vmoodle', $vmoodle);
        }
...
if ($action == 'edit'){
    ...
    if (!($form = get_record('block_vmoodle', 'id', $id))){
...
if ($action == 'delete'){
    ...
    $vmoodle = get_record('block_vmoodle', 'id', $id);

    if ($vmoodle){
        /// drop record in vmoodle table
        delete_records('block_vmoodle', 'id', $id);
        ...
        if ($vmoodle->mnet){
            ...
            $mnetpeers = get_records('block_vmoodle', 'mnet', 1);
            if (!empty($mnetpeers)){
