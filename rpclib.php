...
function mnetadmin_rpc_bind_peer($username, $userhost, $remotehost, $new_peer, $servicestrategy) {
	...
	if($oldpeer = get_record('mnet_host', 'wwwroot', $peerobj->wwwroot)){
	    $peerobj->id = $oldpeer->id;
	    if (!update_record('mnet_host', $peerobj)){
	    ...
	    if (!$peerobj->id = insert_record('mnet_host', $peerobj)){
			$response->status = RPC_FAILURE_RECORD;
	...
	if (!empty($servicestrategy)){
    	foreach($servicestrategy as $servicename => $servicestate){
    	    delete_records('mnet_host2service', 'hostid', $peerobj->id); // eventually deletes something on the way
    	    $service = get_record('mnet_service', 'name', $servicename);
...
function mnetadmin_rpc_unbind_peer($username, $userhost, $remotehost, $peer_wwwroot) {
	...
	if($vmoodle_host = get_record('mnet_host', 'wwwroot', $peer_wwwroot)){
		$vmoodle_host->deleted	=	1;
		if(!update_record('mnet_host', $vmoodle_host)){
			$response->status	=	RPC_FAILURE_RECORD;
 ...
 */
foreach(glob($CFG->dirroot.'/blocks/vmoodle/plugins/libs/*/rpclib.php') as $rpclib){
	require_once $rpclib;
}