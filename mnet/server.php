...
// Extract the XML-RPC payload from the XML-ENC and XML-SIG wrappers.
    $payload = vmoodle_mnet_server_strip_wrappers($HTTP_RAW_POST_DATA);
} else {
 ...
 */
function vmoodle_mnet_server_strip_wrappers($HTTP_RAW_POST_DATA) {
    global $CFG, $MNET, $MNET_REMOTE_CLIENT;
    if (isset($_SERVER)) {

        $crypt_parser = new mnet_encxml_parser();
        $crypt_parser->parse(trim($HTTP_RAW_POST_DATA));
        ...
        if ($signature_verified == 0) {
            ...
            // DO NOT TRY THIS !! key is known having changed in the meanwhile
            
            // Get the key the remote peer is currently publishing:
            $currkey = mnet_get_public_key($MNET_REMOTE_CLIENT->wwwroot, null);
...
function mnet_server_fault($code, $text, $param = null) {
    ...
    debug_trace("$CFG->wwwroot VMOODLE MNET Server fault : $code / $text");
...
function mnet_keyswap($function, $params) {
    ...
    // PATCH : allows forcing for a vmoodling implementation 
    $wwwroot        = $params[0];
    $pubkey         = $params[1];
