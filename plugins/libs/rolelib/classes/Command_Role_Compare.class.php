 ...
 */
class Vmoodle_Command_Role_Compare extends Vmoodle_Command {
	...
	public function __construct() {
		...
		$cmd_name = get_string_vml('cmdcomparename', 'rolelib');
		$cmd_desc = get_string_vml('cmdcomparedesc', 'rolelib');

		// Getting role parameter
		$records = get_records('role', '', '', 'name', 'name,shortname');
		$roles = array();
		foreach($records as $record)
			$roles[$record->shortname] = $record->name;
		$role_param = new Vmoodle_Command_Parameter('role', 'enum', get_string_vml('roleparamcomparedesc', 'rolelib'), null, $roles);
	...
	public function run($hosts) {
		...
		$rpc_client->set_method('blocks/vmoodle/rpclib.php/mnetadmin_rpc_get_role_capabilities');
	...
	private function _process() {
		...
		$records_capabilities = get_records('capabilities', '', '', '', 'name,contextlevel,component');
		...
		$problematic_component_name = get_string_vml('problematiccomponent', 'rolelib');
		...
		// Creating header
		$this->report = '<h3>'.get_string_vml('comparerole', 'rolelib', $role->name).help_button_vml('rolelib', 'rolecompare', 'rolecompare').'</h3>';
		
		// Adding edit role link
		$this->report.= '<center>'.print_single_button($CFG->wwwroot.'/admin/roles/manage.php', array('roleid' => $role->id, 'action' => 'view'), get_string_vml('editrole', 'rolelib'), 'get', '_self', true).'</center>';
		
		// Creation form
		$this->report.= '<form action="'.$CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/controller.rolelib.sadmin.php?what=syncrole" method="post" onsubmit="return validate_syncrole()"><input id="capability" type="hidden" name="capability" value=""/><input id="source_platform" type="hidden" name="source_platform" value=""/>';
		...
		foreach($hosts as $host) {
			$this->report.= '<th id="cap_'.$col.'" scope="col" class="header c'.$col.'" style="vertical-align: bottom; text-align: center;"><label for="platform_'.$col.'"><img src="'.$CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/draw_platformname.php?caption='.urlencode($host_labels[$host]).'" alt="'.$host_labels[$host].'"/></label><br/><input id="platform_'.$col.'" type="checkbox" name="platforms[]" value="'.$host.'" disabled="disabled"/></th>';
			$col++;
		}
		$this->report.= '</tr>';
			...
			foreach($hosts as $host) {
				...
				if (array_key_exists($host, $this->capabilities) && property_exists($this->capabilities[$host], $capability->name)) {
					...
					if (is_null($platform_capability))
						$cell = '<img src="'.$CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/pix/norolecapability.png" alt="No role capability" title="'.$title.'" onclick="setCapability('.$col.','.$row.',\''.$capability->name.'\',\''.$host.'\');"/>';
					else {
						$cell = '<img src="'.$CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/pix/compare'.$cap_permissions[$platform_capability->permission]['label'].$cap_contexts[$platform_capability->contextlevel]['label'].'.png" alt="Permission: '.$cap_permissions[$platform_capability->permission]['name'].' | Context: '.$cap_contexts[$platform_capability->contextlevel]['name'].'" title="'.$title.'" onclick="setCapability('.$col.','.$row.',\''.$capability->name.'\',\''.$host.'\');"/>';
						if ($platform_capability->permission != $capabilities[$platform_capability->capability]->major_permission)
							$extra_class = 'wrongvalue';
						elseif ($platform_capability->contextlevel != $capabilities[$platform_capability->capability]->major_contextlevel) $extra_class = 'wrongcontext';
					}
				} else {
					$cell = '<img src="'.$CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/pix/nocapability.png" alt="No capability" title="'.$title.'"/>';
				}
		...
		// Closing table
		$this->report.= '</tboby></table><br/><center><input type="submit" value="'.get_string_vml('synchronize', 'rolelib').'"/><div id="rolecompare_validation_message"></div></center></form><br/><br/>';
	}
