 ...
 */
class Vmoodle_Command_Role_Capability_Sync extends Vmoodle_Command {
	/**
	 * Constructor.
	...
	public function __construct() {
		...
		$cmd_name = get_string_vml('cmdsynccapabilityname', 'rolelib');
		$cmd_desc = get_string_vml('cmdsynccapabilitydesc', 'rolelib');
		
		// Creating platform parameter
		$platform_param = new Vmoodle_Command_Parameter('platform',	'enum', get_string_vml('platformparamsyncdesc', 'rolelib'), null, get_available_platforms());
		
		// Getting role parameter
		$records = get_records('role', '', '', 'name', 'name,shortname');
		foreach($records as $record)
			$roles[$record->shortname] = $record->name;
		$role_param = new Vmoodle_Command_Parameter('role', 'enum', get_string_vml('roleparamsyncdesc', 'rolelib'), null, $roles);
		
		// Creating capability parameter
		$records = get_records('capabilities', '', '', 'name', 'name');
		$capabilities = array();
		foreach($records as $record)
			$capabilities[$record->name] = get_capability_string($record->name);
		asort($capabilities);
		$capability_param = new Vmoodle_Command_Parameter('capability', 'enum', get_string_vml('capabilityparamsyncdesc', 'rolelib'), null, $capabilities);
	...
	public function run($hosts) {
		...
		$rpc_client = new Vmoodle_XmlRpc_Client();
		$rpc_client->set_method('blocks/vmoodle/rpclib.php/mnetadmin_rpc_set_role_capabilities');
		$rpc_client->add_param($role, 'string');
	...
	public function getResult($host=null, $key=null) {
		...
		if (is_null($host))
			if (isset($SESSION->vmoodle_sa['rolelib']['command']) && isset($SESSION->vmoodle_sa['rolelib']['platforms'])) {
				return '<center>'.print_single_button($CFG->wwwroot.'/blocks/vmoodle/plugins/libs/rolelib/controller.rolelib.sadmin.php', array('what' => 'backtocomparison'), get_string_vml('backtocomparison', 'rolelib'), 'get', '_self', true).'</center><br/>';
			} else
				return null;
