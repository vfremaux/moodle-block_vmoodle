 ...
 */
class Vmoodle_Command_Parameter_Internal extends Vmoodle_Command_Parameter {
	...
	public function __construct($name, $function, $parameters=null) {
		...
		if (!function_exists($function)) {
			throw new Vmoodle_Command_Exception('parameterinternalfunctionnotexists', (object) array('function_name' => $function, 'parameter_name' => $this->name));
		} else
			$this->fct = $function;
	...
	public function retrieveValue($datas=null) {
		...
		if (!is_null($this->parameters)) {
			foreach ($this->parameters as $parameter) {
				preg_match_all(Vmoodle_Command::placeholder, $parameter, $vars);
				...
				foreach ($vars[2] as $key => $var) {
					if (empty($vars[1][$key]) && !array_key_exists($var, $vmcommands_constants)) {
						throw new Vmoodle_Command_Exception('parameterinternalconstantnotgiven', (object)array('constant_name' => $var, 'parameter_name' => $this->name));
					} else if (!empty($vars[1][$key]) && !array_key_exists($var, $datas))
						throw new Vmoodle_Command_Exception('parameterinternalparameternotgiven', (object)array('parameter_need' => $var, 'parameter_name' => $this->name));
				}
			}
			// Replace placeholders by theirs values
			$this->datas = $datas;
			$this->parameters = preg_replace_callback(Vmoodle_Command::placeholder, array($this, '_replaceParametersValues'), $this->parameters);
			unset($this->datas);
		}
		...
		try {
			...
			throw new Vmoodle_Command_Exception(
